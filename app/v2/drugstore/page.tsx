...
const PROBLEMS = [
  ...
  },
  {
    ...
    body: 'LINE公式アカウント単体の一斉配信はブロック率20〜30%が業界水準。調剤完了通知を起点にした関係性配信は、この数値を大幅に下げられます。',
  },
  {
...
const APPEAL_STEPS = [
  {
    step: 'Step 1',
    title: '調剤完了通知',
    description: '処方箋受付後、調剤が完了したタイミングでLINEに通知を送信。「呼ばれるまで待つ」ストレスを解消し、ブロック率を業界指標2.5〜7%（LINE公式単体20〜30%比）まで低減します。',
  },
  {
    step: 'Step 2',
    title: '待ち時間クーポン配信',
...
const PHASES = [
  {
    ...
    label: '顧客接点の創出',
    features: ['デジタル会員証'],
    description: 'レジ前5秒でLINE会員化。ポイントカードとアプリDLの障壁を取り除き、会員基盤を広げる土台を作ります。',
  },
  {
    phase: 'Phase 2',
    label: '接客と調剤の連携',
    features: ['スタンプカード', 'クーポン配信', '1to1コミュニケーション'],
    description: '調剤完了通知・待ち時間クーポン・事前受付の3層で、調剤待ちを接点に変えます。スタンプとクーポンで再来店も設計します。',
  },
  {
    phase: 'Phase 3',
    label: '関係性の深化',
    features: ['セグメント配信', 'ギフト'],
    description: 'POSデータ×購買属性でセグメントを切り、配信精度を上げます。ギフトで紹介獲得も循環させます。',
  },
];
    ...
    <main className="min-h-screen bg-white text-[#1F2937]">
      ...
      <header className="sticky top-0 z-50 bg-white/95 backdrop-blur border-b border-[#E5E7EB]">
        <div className="max-w-[1200px] mx-auto px-4 sm:px-5 md:px-6 h-16 md:h-20 flex items-center justify-between">
          <Link href="/v2/drugstore" className="flex items-center gap-2">
      ...
      <Section id="phases" spacing="md" container="wide" background="muted">
        <div className="max-w-[720px] mb-10 md:mb-12">
          ...
          <p className="text-base text-[#4B5563]">会員基盤の構築→調剤との連携→購買データ活用。各フェーズが次のデータ基盤になります。</p>
        </div>
