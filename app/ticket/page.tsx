...
const FAQS = [
  {
    q: 'スポーツ・エンタメ以外の興行でも利用できますか？',
    a: '対応可能です。スポーツ観戦・コンサート・展示会・体験型イベントなど、入場管理が必要な興行全般でご利用いただいています。屋外フェスや分散型イベントにも対応できます。',
  },
  {
    q: 'もぎり方式はどのような種類がありますか？',
    a: 'スタッフがスキャナーで読み取る方式、来場者が自分でQRを提示してチェックする方式、会員IDと連動して入場を自動判定する方式に対応しています。イベント規模や運営体制に合わせてご提案します。',
  },
  {
    q: '先行抽選はどのような仕組みですか？',
    a: 'LINE友だちや会員ランクを参加条件に設定し、申込期間内にエントリーを受け付けます。抽選後は当落通知と同時にチケットを自動発行。繰り上げ当選や補欠管理も設定可能です。',
  },
  {
    q: '開発期間と費用の目安は？',
    a: 'チケット発行・QR入場チェックの基本構成であれば最短3ヶ月で立ち上げが可能です。先行抽選・複数時間枠管理・予約との連携など要件により期間と費用が変わるため、初回ヒアリングで概算をご提示します。',
  },
  {
    q: 'どのような業界で活用されていますか？',
    a: 'スポーツ観戦（クラブ・アリーナ）、エンタメ（コンサート・演劇）、展示会・見本市、商業施設のイベントなど、入場管理とファン育成の両方が必要な事業会社さまで活用いただいています。',
  },
];
  ...
  step: [
    {
      '@type': 'HowToStep',
      name: '要件ヒアリング・見積',
      text: 'イベント規模・時間枠管理の粒度・消込方式・予約との連携要否を確認し、概算費用と工期を提示します。',
      position: 1,
    },
    {
      '@type': 'HowToStep',
      name: '設計・実装',
      text: 'チケット発行・QR入場チェック・転売防止・会員ID連携の設計と実装を行います。',
      position: 2,
    },
    {
      '@type': 'HowToStep',
...
const articleJsonLd = {
  '@context': 'https://schema.org',
  '@type': 'Article',
  headline: 'LINEチケット｜デジタル発券・もぎり・先行抽選を1つに｜グロースパック for LINE',
  description: '紙チケットの印刷コスト、当日もぎり作業の人手不足、先行販売の管理煩雑さ。LINEでデジタル発券・もぎり・先行抽選を一元化。ハーフスクラッチ開発で最短3ヶ月。',
    ...
    <main className="min-h-screen bg-white text-foreground">
          ...
          <div className="flex flex-col md:flex-row items-center gap-8 md:gap-12">
            <div className="flex-1 max-w-[600px] space-y-6 md:space-y-7">
              ...
              <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold leading-[1.15] tracking-tight text-foreground">
                並ばなくていい、偽造されない、<br />
                そして終わった後も<br />
                <span className="text-line-green-dark">つながれる。</span>
              </h1>
              <p className="text-base sm:text-lg text-foreground/80 leading-relaxed max-w-[600px]">デジタルチケット発行と同時に会員ID取得。QR入場チェック・転売防止・時間枠管理で、紙整理券を完全に置き換えます。</p>
      ...
      <Section id="contact" spacing="md" container="wide" background="dark">
        ...
        <div className="text-center space-y-6 md:space-y-8">
          ...
          <p className="text-base sm:text-lg text-white/80 max-w-[640px] mx-auto leading-relaxed">イベント規模・時間枠管理の要件・予約との統合要否をお聞きして、最適な構成をご提案します。<span className="font-bold text-white">初回相談は無料です。</span></p>
      ...
      <LpFooter
        description="クラスメソッド株式会社が提供するLINEミニアプリ開発サービス。チケット発行を顧客獲得の起点に変えます。"
        columns={[
          {
            heading: 'SERVICE',
            links: (
