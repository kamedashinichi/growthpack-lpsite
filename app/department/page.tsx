    ...
    <main className="min-h-screen bg-white text-foreground">
              ...
              <Link key={f.id} href={f.url} target="_blank" rel="noopener noreferrer" className="group block h-full rounded-xl outline-none focus-visible:ring-2 focus-visible:ring-link focus-visible:ring-offset-2">
                <Card padding="md" className="h-full flex flex-col border-2 border-border group-hover:border-line-green transition-colors">
                  <div className="flex flex-wrap items-center gap-2 mb-3">
                    <span className={`inline-block text-sm font-semibold px-2.5 py-1 rounded-full whitespace-nowrap ${phaseColor}`}>
                      {f.phase}
                    </span>
                    <span className={`inline-block text-sm font-semibold px-2.5 py-1 rounded-full whitespace-nowrap ${layerColor}`}>
                      {f.layer}
                    </span>
                  </div>
                  <div className="flex items-start gap-4 mb-3">
        ...
        <div className="bg-white rounded-2xl overflow-hidden border border-border shadow-sm">
          <div className="flex flex-col md:flex-row">
            ...
            <div className="p-6 sm:p-8 md:p-10 flex flex-col justify-center md:w-3/5">
              ...
              <WPDownloadButton industry="department" />
            </div>
          </div>
        </div>
      </Section>
      ...
      <Section id="contact" spacing="lg" container="default" background="dark">
        <div className="text-center space-y-6 md:space-y-8">
          ...
          <p className="text-base sm:text-lg text-white/80 max-w-[640px] mx-auto leading-relaxed">外商規模・催事頻度・既存CRMをお聞きして最適な構成をご提案します。<span className="font-bold text-white">初回相談は無料です。</span></p>
      ...
      {/* ============================================================ */}
      <LpFooter
        description="クラスメソッド株式会社が提供する LINE ミニアプリ開発サービス。百貨店・商業施設の外商二層設計・催事デジタル化・館内ID統合に対応します。"
        columns={[
          {
            heading: 'SERVICE',
            links: (
